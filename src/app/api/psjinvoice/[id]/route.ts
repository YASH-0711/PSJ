...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await PsjInvoice.findByIdAndUpdate(
      id,
      { $set: updateData },
    ...
    console.error("PUT /api/psjinvoice/[id] error:", error.message);
    return NextResponse.json({ message: error.message }, { status: 500 });
  }
}
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    await connect();
    const { id } = await params;
    const deleted = await PsjInvoice.findByIdAndDelete(id);
    if (!deleted) return NextResponse.json({ message: "Record not found" }, { status: 404 });
    return NextResponse.json({ success: true, message: "Deleted successfully" });
  } catch (error: any) {
    console.error("DELETE /api/psjinvoice/[id] error:", error.message);
    return NextResponse.json({ message: error.message }, { status: 500 });
  }
}
